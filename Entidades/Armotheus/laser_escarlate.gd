...
const SWEEP_SPEED     := 150.0
const PARRY_RANGE     := 72.0
const CONTACT_DAMAGE  := 1800.0

enum Phase { WARN, SWEEP, DONE }

var _player         : Node2D = null
var _fire_scene     : PackedScene = null

var _arena_left   : float = 0.0
var _arena_right  : float = 0.0
var _arena_floor  : float = 0.0
...
func _ready() -> void:
	...
	# Conecta ao EventBus para escutar counter do player
	...
	_sweep_target = _player.global_position.x if _player else (_arena_left + _arena_right) * 0.5
...
func _tick_sweep(delta: float) -> void:
	_sweep_x += _sweep_dir * SWEEP_SPEED * delta

	var reached : bool = (
		(_sweep_dir > 0.0 and _sweep_x >= _sweep_target) or
		(_sweep_dir < 0.0 and _sweep_x <= _sweep_target)
	)
	...
	if reached:
		...
		if _player and is_instance_valid(_player):
			dashing = _player.get("is_dashing")
		
		if not dashing and _player and is_instance_valid(_player):
...
func _disconnect_counter() -> void:
	if EventBus.is_connected("player_counter_pressed", _on_counter):
		EventBus.disconnect("player_counter_pressed", _on_counter)
